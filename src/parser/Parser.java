...
public class Parser {
    ...
    private void constInitVal() {
        ...
        if (words.getNextKind() == Token.LBRACE) {
            ...
            if (words.getNextKind() != Token.RBRACE) {
                do {
                    if (words.getNextKind().equals(Token.COMMA)) {
                        terminalTk(Token.COMMA);
                    }
    ...
    private void stmt() {
        ...
        switch (words.getNextKind()) {
            case IDENFR:  //assignStmt or getint or exp
                if (words.getNthKind(1).equals(Token.LPARENT)) { // exp 中的函数调用
                    exp();
                    terminalTk(Token.SEMICN);
                } else { // primary exp / assign / getInt
                    words.saveEnv();
                    lVal();
                    if (words.getNextKind().equals(Token.ASSIGN)) {
                        builder.goBack(1);
                        if (words.getNthKind(1).equals(Token.GETINTTK)) {
                            words.loadEnv();
                            getIntStmt();
                        } else {
                            words.loadEnv();
    ...
    private void returnStmt() {
        ...
        if (!words.getNextKind().equals(Token.SEMICN) && (words.getNextWord().getLine() == words.getNthWord(-1).getLine())) {
            exp();
        }
        terminalTk(Token.SEMICN);
        builder.endBuild(node);
        ...
        switch (words.getNextKind()) {
            case IDENFR:
                if (words.getNthKind(1).equals(Token.LPARENT)) {
                    terminalTk(Token.IDENFR);
                    terminalTk(Token.LPARENT);
                    if (words.getNextKind().equals(Token.IDENFR) || words.getNextKind().equals(Token.INTCON)
                            || words.getNextKind().equals(Token.LPARENT) || words.getNextKind().equals(Token.PLUS)
                            || words.getNextKind().equals(Token.MINU) || words.getNextKind().equals(Token.NOT)) {
                        funcRParams();
                    }
                    terminalTk(Token.RPARENT);
                } else {
    ...
    private void terminalTk(Token token) {
        if (token.equals(words.getNextKind())) {
            ...
            // Semicolon_Dismiss
            ErrorKind errorKind;
            int line = words.getNthWord(-1).getLine();
            switch (token) {
                case SEMICN:
                    errorKind = ErrorKind.Semicolon_Absence;
                    break;
                case RPARENT:
                    errorKind = ErrorKind.RParent_Absence;
                    break;
                case RBRACK:
                    errorKind = ErrorKind.RBrack_Absence;
                    break;
                default:
                    errorKind = ErrorKind.Unknown_Error;
                    break;
            }
            builder.buildError(errorKind,line);
        }
    }
